...
export function ModalDetalhes({ isOpen, onClose, produto, onConfirm, }) {
    ...
    const loadHistorico = async () => {
        try {
            const res = await fetch(`http://localhost:8000/api/historico-confirmacao/${produto.id_item}`);
    ...
    return (<div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="product-info">
            ...
            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #1e40af' }}>
              ...
              <div className="nf-list">
                {produto.notas_fiscais.map((nf, idx) => (<div key={idx} className="nf-badge">
          ...
          {message && (<div className={messageType === 'success' ? 'success-message' : 'error-message'}>
              {message}
            </div>)}
          ...
          <div className="confirmation-section">
            ...
            <div className="form-group">
              ...
              <input type="number" className="form-input" value={quantidadeConfirmada} onChange={(e) => setQuantidadeConfirmada(parseFloat(e.target.value) || 0)} step="0.01"/>
            </div>

            {divergenciaValor !== 0 && (<div style={{
            ...
            <div className="form-group">
              <div className="checkbox-group">
                <div className="checkbox-item">
                  <input type="checkbox" id="temDivergencia" checked={temDivergencia} onChange={(e) => setTemDivergencia(e.target.checked)}/>
                  <label htmlFor="temDivergencia">Registrar divergência</label>
                </div>
              </div>
            </div>

            {temDivergencia && (<div className="form-group">
                <label className="form-label">Tipo de Divergência</label>
                <select className="form-select" value={divergencia} onChange={(e) => setDivergencia(e.target.value)}>
            ...
            <div className="form-group">
              ...
              <textarea className="form-input" value={observacoes} onChange={(e) => setObservacoes(e.target.value)} placeholder="Digite aqui qualquer observação adicional..." rows={3} style={{ resize: 'vertical' }}/>
            </div>
          </div>
          ...
          {/* Histórico */}
          {historico.length > 0 && (<div className="historico-section">
              <h3>Histórico de Confirmações</h3>
              {historico.map((item) => (<div key={item.id} className="historico-item">
                  ...
                  {item.divergencia && (<p className="historico-divergencia">
                      Divergência: {item.divergencia}
                    </p>)}
                  {item.observacoes && (<p style={{ color: '#999', fontSize: '0.8rem' }}>
                      {item.observacoes}
                    </p>)}
                </div>))}
            </div>)}
