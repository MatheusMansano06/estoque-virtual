...
interface KitInfo {
  ...
  sku_kit: string
  nome_kit: string
  skus_componentes: string[]
  quantidade_componentes: number
  id_kit: number
}

interface BuscadorKitProps {
  itemId: number
  onKitDetectado?: (kit: KitInfo, componentes: Componente[]) => void
  onSemKit?: () => void
}

export default function BuscadorKit({ itemId, onKitDetectado, onSemKit }: BuscadorKitProps) {
  const [skuBuscado, setSkuBuscado] = useState('')
  const [buscando, setBuscando] = useState(false)
  const [kitDetectado, setKitDetectado] = useState<KitInfo | null>(null)
  const [componentes, setComponentes] = useState<Componente[]>([])
  const [quantidades, setQuantidades] = useState<{[key: string]: number}>({})
  const [mensagem, setMensagem] = useState<{tipo: 'info' | 'sucesso' | 'erro', texto: string} | null>(null)
  ...
  return (
    <div style={{ padding: '1.5rem', background: '#fff3cd', border: '3px solid #ffc107', borderRadius: '8px', marginBottom: '2rem' }}>
      ...
      <form onSubmit={handleBuscar} style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
        <input
          type="text"
          placeholder="Digite o SKU do kit (ex: V+RL3)"
          value={skuBuscado}
      ...
      {kitDetectado && componentes.length > 0 && (
        <div style={{ background: '#fff', border: '2px solid #4caf50', borderRadius: '6px', padding: '1rem' }}>
